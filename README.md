...
In the example below the relations Tags & PageBlocks would need their own searchable_* config. When indexing, these relationships will be traversed and a nested objects created.
...
## Creating a search index instance
...
Calling this method will build an ES endpoint index. For full configuration for mappings and settings please see the Elastic Search documentation.
...
The easiest way to create an index is to create a SilverStripe task and run it to create / rebuild all your indexes
...
### Configuring Your Search Queries
